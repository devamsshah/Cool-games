...
void initField(int num_of_mine) {
    //Complete me
    for (int i = 0; i < height; i++) {//goes through each row
        for (int j = 0; j < width; ++j) {//goes through each cell in teh row
            setCell(i, j,UNKNOWN);//sets the cell to unknown
        }
    }
    setMine(num_of_mine);//sets the mines randomly by calling a preciously established function
}
...
bool isMine(int i, int j) {
    ...
    if((getCell(i,j) == UNFLAGGED_MINE)||(getCell(i,j)==FLAGGED_MINE)){
        return true;//returns the value of mine to be true if its a flagged or an unflagged mine
    }
...
void reveal(int i,int j) {
    //Complete me
    int minecount = 0;
    if ((i - 1 >= 0 && j - 1 >= 0)) { //if statements that check if the the mine is a corner mine or not and if it is, then
            if (isMine(i - 1, j - 1)==true) {// then the if statement only executes for the i,j which exist in the frame.
                minecount += 1;
            }
        }
        if (j - 1 >= 0) {
        ...
        if((i+1<=7 && j+1<=4)) {
            ...
            setCell(i, j, minecount);// after all the cells have been checked, it returns the value of the number of mines and then sets that number to the particular cell
        //after we check all the surrounding fields for mines, we set the cell we just selected to teh number of mines that are there
}
...
void executeCmd(string cmd) {
    //Comeplete me
    char first = cmd[0]; // these are the first, second and third characters of the string that is going to be input.
    int second = cmd[1]-'0';
    int third = cmd[2]-'0';

    if ((first!='f')&&(first!='r')&&(first!='u')){//checks if the input is valid or not
        cout << "invalid input.";
        return;
    }
    if ((second<0)||(third <0)||(second>7)||(third>4)){//checks if the input is within the frame or not
        cout << "invalid input.";
        return;
    }
    if(first == 'f'){//checks if the input starts with an 'f' meaning if the player wants to flag the mine or not
        if (getCell(second, third)== UNFLAGGED_MINE){ //checks if the cell is unflagged or not and then it assignes it to flagged if it is an unflaggedd mine
            setCell(second, third, FLAGGED_MINE);
            num_correct_flagged_mine++;//increases the counter of number of correctly flagged mines
        }else {
            setCell(second, third, INCORRECT_FLAGGED_MINE);//if it is not a flagged mine, it marks it as incorrectly flagged mine and increases the counter by one
            num_incorrect_flagged_mine++;
        }
    }
    if (first == 'u'){//checks if the user wants to unflag the cell or not.
        if (getCell(second, third) == FLAGGED_MINE){ //checks if its a flagged cell
            setCell(second, third, UNFLAGGED_MINE);//sets it to unflagged cell
            num_correct_flagged_mine--;//decreases the flagged mine counter
        }else if (getCell(second, third) == UNKNOWN){//checks if its an unknown mine and sets it to unknown
            setCell(second, third, UNKNOWN);
            num_incorrect_flagged_mine--;//decreases the incorrectly flagged mine counter
        }
    }

    if(first=='r'){//checks if the player wants to reveal the cell
        if(isMine(second, third)){//checks if its mine to determine if the player lost or not.
            explode = true;
        }
        else{//if the player didnt lose then the cell is revealed. 
            reveal(second, third);
        }
    }
